...
from scripts.notion.notion_py.block import TextBlock
...
def test_block_update_with_collection_block(notion_token):
    ...
    updated_block = notion_api.block_update(
        block.id, {"name": "test block has been updated", "value": 5}
    )
...
def test_collection_view_append(notion_token):
    ...
    notion_api.collection_append(
        collection_id, view_id, {"enabled": True, "value": 10, "name": "test row"}
    )
